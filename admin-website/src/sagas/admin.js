...
export function* generateHash(certficateString){
  var certString = JSON.parse(JSON.stringify(certficateString)).payload.certString;

  try {    
    const web3 = yield getSelectedWeb3();    
    const certHash = yield web3.utils.keccak256(certString)
    console.log("CertHash admin.js-"+ certHash);
    yield put({
      type: types.GENERATING_CERT_HASH_SUCCESS,
      payload: {
        certHash: certHash        
      }
    });
    ...
    yield put({
      type: types.GENERATING_CERT_HASH_FAILURE,
      payload: e.message
    });
    error("GENERATECERTHASH:", e);
  }
}
